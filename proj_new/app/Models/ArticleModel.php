...
{ 
	...
	{ 
		...
		if($option['task']=='news-list-items') {
			$query =  $this->select('a.id','a.name','a.category_id')->leftJoin('category as c','a.category_id','=','c.id')->where('a.is_home','=','1')->limit(8); 
			$result = $query->get()->toArray();
		} 

		if($option['task']=='news-list-items-is-home') {
			$query =  $this->select('a.id','a.name','a.category_id','a.display')
			->leftJoin('category as c','a.category_id','=','c.id')->where('a.status','=','active')->limit(8); 
			$result = $query->get()->toArray();
		} 
		...
		if($option['task']=='news-list-items-latest') {
			$query =  $this->select('a.id','a.name','a.thumb','a.category_id','a.created','c.name as category_name')
			->leftJoin('category as c','a.category_id','=','c.id')
			->where('a.status','=','active')
			->orderBy('a.id', 'asc')
			->take(4);  
		...
		if($option['task']=='news-list-items-in-category') {
 			$query =  $this->select('a.id','a.name','a.content','a.thumb','a.created','a.category_id')
			->leftJoin('category as c','a.category_id','=','c.id')
			->where('a.status','=','active')->where('a.category_id','=',$params['category_id'])
			->take(4);
			$result = $query->get()->toArray();
		} 

		if($option['task']=='news-list-items-related-in-category') {
 			$query =  $this->select('id','name','content','thumb','category_id','a.created')
 			->where('a.id','!=',$params['article_id'])->where('category_id','=',$params['category_id'])
			->take(4);
			$result = $query->get()->toArray();
		} 
		return $result;
	...
	public function getItem($params=null, $option=null)
	{
		...
		if($option['task']=='news-get-item') {   
			$result = self::select('a.id','a.name','a.content','a.category_id','a.thumb','c.name as category_name','a.created','c.display')
			->leftJoin('category as c','a.category_id','=','c.id')
